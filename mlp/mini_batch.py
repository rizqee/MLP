...
class MiniBatch:
    def __init__(
        ...
        self.mlp = MultilayerPerceptron(
            [input_layer] + hidden_layer + [self.target_layer], learning_rate
        )
    ...
    def fit(self):
        ...
        iteration_num = 0
        while (
            iteration_num == 0 or self.mlp.error() > self.error_threshold
        ) and iteration_num <= self.max_iteration:
            for _ in range(min(self.mini_epoch_num, len(self.input) - i)):
